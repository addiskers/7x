...
class MediaHandler {
  ...
  // Seconds of agent audio already received but not yet played. nextStartTime is the
  // AudioContext clock at which the last scheduled buffer ends, so this lets the UI
  // hold the call screen until the closing has actually finished playing.
  remainingPlaybackSeconds() {
    if (!this.audioContext || this.audioContext.state === "closed") return 0;
